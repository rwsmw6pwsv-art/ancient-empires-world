#!/usr/bin/env python3
"""Named region moves and new hex seats. Tessellate INSET 0.99."""
...
def main() -> None:
    ...
    wed = by_id["weddell"]
    toc = by_id["tocantins"]
    sa_a = take(wed["q"] - 1, wed["r"], "sa-weddell-w")
    sa_b = take(toc["q"], toc["r"] + 1, "sa-tocantins-se")

    ori = by_id["orinoco"]
    ara = by_id["arawak"]
    ca_mid = take(ori["q"] + 1, ori["r"] - 1, "ca-orinoco-arawak")
    assert (ca_mid[0] - ara["q"], ca_mid[1] - ara["r"]) in {
        (1, 0), (-1, 0), (0, 1), (0, -1), (1, -1), (-1, 1),
    }

    hej = by_id["hejaz"]
    me_e = take(hej["q"] + 1, hej["r"], "me-hejaz-e")

    jut = by_id["jutland"]
    eu_e = take(jut["q"] + 1, jut["r"], "eu-jutland-e")

    kur = by_id["kuril"]
    ae_e1 = take(kur["q"] + 1, kur["r"], "ae-kuril-e")
    ae_e2 = take(kur["q"] + 2, kur["r"], "ae-kuril-ee")

    new_lands = [
        (sa_a, "sa", "drake", "Drake", "tundra"),
        (sa_b, "sa", "fuegia", "Fuegia", "tundra"),
        (ca_mid, "ca", "paria", "Paria", "jungle"),
        (me_e, "me", "najd", "Najd", "desert"),
        (eu_e, "eu", "gotland", "Gotland", "forest"),
        (ae_e1, "ae", "iturup", "Iturup", "forest"),
        (ae_e2, "ae", "kunashir", "Kunashir", "forest"),
    ]
    ...
    print("total", len(recs), "added", added)
    ...
    for pid in ("peninsula", "weddell", "nord", "gobi", "orinoco", "atlas", "volcan"):
        assert pid in by_id, pid
    assert by_id["liguria"]["continent"] == "me"
    assert by_id["sierra"]["continent"] == "nw"
    assert by_id["nicoya"]["continent"] == "nw"
    assert by_id["tequesta"]["continent"] == "ne"
    assert by_id["timucua"]["continent"] == "ne"
    assert by_id["atlas"]["continent"] == "an"
    assert by_id["amur"]["continent"] == "ae"
    assert by_id["baikal"]["continent"] == "ae"
    ...
    assert tuple(sorted(("orinoco", "paria"))) in land_edges
    assert tuple(sorted(("arawak", "paria"))) in land_edges
    assert tuple(sorted(("hejaz", "najd"))) in land_edges
    assert tuple(sorted(("jutland", "gotland"))) in land_edges
    assert tuple(sorted(("kuril", "iturup"))) in land_edges
    assert tuple(sorted(("iturup", "kunashir"))) in land_edges
