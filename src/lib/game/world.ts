...
import { TRIBAL_LANDS, TRIBAL_LAND_EDGES, TRIBAL_SEA_EDGES } from "./tribes.gen";
import { FLOODED_SET } from "./flooded.gen";
import { NAME_OF, REGION_OF } from "./regions.gen";

export const WORLD_W = 1680;
export const WORLD_H = 1260;

const CORE_TERRITORIES: TerritoryDef[] = [
  {
    "id": "amundsen",
    "name": "Firn",
    "continent": "at",
...
const RAW_TERRITORIES: TerritoryDef[] = [...CORE_TERRITORIES, ...TRIBAL_LANDS].filter(
  (t) => !FLOODED_SET.has(t.id),
);

export const CORE_NAMES: Record<string, string> = Object.fromEntries(
  RAW_TERRITORIES.map((t) => [t.id, t.name]),
);

export const TERRITORIES: TerritoryDef[] = RAW_TERRITORIES.map((t) => {
  const continent = REGION_OF[t.id] ?? t.continent;
  const name = NAME_OF[t.id] ?? t.name;
  if (continent === t.continent && name === t.name) return t;
  return { ...t, continent, name };
});
